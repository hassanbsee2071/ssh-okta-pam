...
#define DEVICE_AUTHORIZE_URL  "https://sso.syed.com/oauth2/v1/device/authorize"
#define TOKEN_URL "https://sso.syed.com/oauth2/v1/token"
#define CLIENT_ID "1kx3gvbf4lg9cqfrf190"
...
char * extractEmail(char * str) {
    char *at_sign_ptr;
    char *username;
    at_sign_ptr = strchr(str, '@');
        if (at_sign_ptr != NULL) {
            int username_len = at_sign_ptr - str;
            username = (char *)malloc(username_len + 1);
            if (username == NULL) {
                perror("Failed to allocate memory");
            }
        strncpy(username, str, username_len);
        username[username_len] = '\0';
    }else {
        printf("Invalid email format: No '@' symbol found.\n");
    }
...
PAM_EXTERN int pam_sm_authenticate( pam_handle_t *pamh, int flags,int argc, const char **argv ) {
	...
	sprintf(postData, "client_id=%s&scope=openid profile offline_access groups", CLIENT_ID); 
	...
	char * resp;
		...
		strcpy(str1, chunk.memory);
            ...
            char * idtoken_copy = strdup(idtoken);
			char * header = strtok(idtoken, ".");
			char * payload = strtok(NULL, ".");
			char * decoded = base64decode(payload, strlen(payload));
			char * name = getValueForKey(decoded, "preferred_username");
            char * extracted_username = extractEmail(name); 
            char filename[256];
            snprintf(filename, sizeof(filename), "/tmp/%s", extracted_username);

            FILE *fp = fopen(filename, "w");
            if (fp != NULL) {
            fprintf(fp, "%s", idtoken_copy);
            fclose(fp);
            chmod(filename, 0600);  // Set secure permissions
            } else {
            fprintf(stderr, "Error: Could not write token to file %s\n", filename);
            }
